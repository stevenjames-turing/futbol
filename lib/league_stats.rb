...
module LeagueStats
  ...
  def goal_data_by_home_team
    @games_data.each do |row|
      if !@goals_per_team.has_key?(row.home_team_id)
        @goals_per_team[row.home_team_id] = {goals_while_away: 0, goals_while_home: row.home_goals.to_i, total_goals: row.home_goals.to_i,
          away_games_played: 0, home_games_played: 1, total_games_played: 1}
  ...
  def goal_data_by_away_team
    @games_data.each do |row|
      if !@goals_per_team.has_key?(row.away_team_id)
        @goals_per_team[row.away_team_id] = {goals_while_away: row.away_goals.to_i, goals_while_home: 0, total_goals: row.away_goals.to_i,
          away_games_played: 1, home_games_played: 0, total_games_played: 1}
  ...
  def goal_data_by_team
    goal_data_by_away_team
    goal_data_by_home_team
    goal_data_averages
    @goals_per_team
  end
  ...
  def highest_scoring_visitor
    ...
    goal_data_by_team.each_pair do |team, data|
      if data[:avg_goals_per_game_away] > highest_away
        (highest_away = data[:avg_goals_per_game_away]) &&(highest_scoring_visitor = data[:team_name])
      end
    end
  ...
  def highest_scoring_home_team
    ...
    goal_data_by_team.each_pair do |team, data|
      if data[:avg_goals_per_game_home] > highest_home
        (highest_home = data[:avg_goals_per_game_home]) && (highest_scoring_home = data[:team_name])
      end
    end
  ...
  def lowest_scoring_visitor
    ...
    goal_data_by_team.each_pair do |team, data|
      if data[:avg_goals_per_game_away] < lowest_away
        (lowest_away = data[:avg_goals_per_game_away]) && (lowest_scoring_visitor = data[:team_name])
      end
    end
  ...
  def lowest_scoring_home_team
    ...
    goal_data_by_team.each_pair do |team, data|
      if data[:avg_goals_per_game_home] < lowest_home
        (lowest_home = data[:avg_goals_per_game_home]) && (lowest_scoring_home = data[:team_name])
      end
    end
    lowest_scoring_home
  end
